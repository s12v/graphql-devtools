...
var har = {
  ...
  /*
   * The request as a fetch() the page can make again: { url, method, headers, body }. With a body
   * given, that one is sent instead of the recorded one (Edit & resend).
   */
  ...
  /* the replay as a statement for chrome.devtools.inspectedWindow.eval — runs in the page, with its cookies */
  replayCode : function (spec) {
    return "(function (r) { fetch(r.url, { method: r.method, headers: r.headers, body: r.body, credentials: \"include\" }).catch(function () {}); })(" + JSON.stringify(spec) + ")";
  },
