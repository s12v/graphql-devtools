...
var graphql = {
  ...
  /*
   * The document re-printed the usual way — one selection per line, two-space indent, arguments
   * and values inline, a blank line between definitions, comments kept. Minified documents
   * (Relay, urql, gql.tada) become readable; an already tidy one comes out the same.
   */
  print : function (src) {
    var tokens = this.tokens(src).filter(function (t) { return t.type != "space"; });
    var out = [];
    var line = "";
    var indent = 0;
    var stack = [];          // "{" selection set · "(" "[" "v" inline contexts (arguments, lists, object values)
    var prev = null;
    var afterSpread = false; // "... on Type" stays on the spread's line

    var pad = function () { return new Array(indent + 1).join("  "); };
    var trimEnd = function () { line = line.replace(/\s+$/, ""); };
    ...
    var inSelection = function () {
      return stack.length > 0 && stack[stack.length - 1] == "{";
    };
    var sep = function () {
      return line === "" || /[(\[{$@ ]$/.test(line) ? "" : " ";
    };
    // commas are whitespace to the lexer; inline items get them back: (a: 1, b: 2), [1, 2], {x: 1, y: 2}
    var comma = function () {
      if (!inline() || !prev) return "";
      var ends = prev.type == "name" || prev.type == "string" || prev.type == "number" || (prev.type == "punct" && /^[)\]}!]$/.test(prev.text));
      return ends ? "," : "";
    };
    var definition = function () {   // a blank line between top-level definitions
      newline();
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        switch (t.text) {
          case "{":
            if (inline()) {
              ...
              if (stack.length == 0 && line === "") definition();
              stack.push("{");
              trimEnd();
              line += (line === "" ? "" : " ") + "{";
              newline();
          ...
          case "(":
            stack.push("(");
            trimEnd();
            // "query ($a: Int)" for an anonymous operation with variables
            line += (prev && prev.type == "name" && stack.length == 1 && /^(query|mutation|subscription)$/.test(prev.text) ? " (" : "(");
            break;
          case "[": stack.push("["); line += comma() + sep() + "["; break;
          case ")": stack.pop(); trimEnd(); line += ")"; break;
          case "]": stack.pop(); trimEnd(); line += "]"; break;
          case ",": if (inline()) { trimEnd(); line += ", "; } break;
          case ":": trimEnd(); line += ": "; break;
          case "!": trimEnd(); line += "!"; break;
          case "=": trimEnd(); line += " = "; break;
          case "|": trimEnd(); line += " | "; break;
          case "&": trimEnd(); line += " & "; break;
          case "...":
            ...
            afterSpread = true;
            break;
          default:                 // $ and @ glue to the name that follows
            line += (t.text == "$" ? comma() : "") + sep() + t.text;
        }
        prev = t;
        continue;
      }
      ...
      if (stack.length == 0 && t.type == "name" && /^(query|mutation|subscription|fragment)$/.test(t.text)) {
        definition();
        line = t.text;
      } else if (inSelection()) {
        var glued = prev && prev.type == "punct" && (prev.text == ":" || prev.text == "..." || prev.text == "@" || prev.text == "$");
        var condition = afterSpread && ((prev && prev.type == "punct" && prev.text == "...") || (prev && prev.type == "name" && prev.text == "on"));
        if (glued || condition) {
          line += (prev && prev.text == "..." && t.text != "on" ? "" : sep()) + t.text;
        } else {
          newline();
          line = pad() + t.text;
          afterSpread = false;
